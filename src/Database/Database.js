...
export default class ItemDatabase {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Item LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, descricao varchar(30), ano varchar(30), prioridade varchar(30), situacao varchar(20))');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Item', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            const { id, descricao, ano, prioridade, situacao } = row;
                            lista.push({ id, descricao, ano, prioridade, situacao });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO Item (descricao, ano, prioridade, situacao) VALUES (?, ?, ?, ?)', [item.descricao, item.ano, item.prioridade, item.situacao]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
    ...
    Atualizar(id, string) {   //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para atualizar um registro no banco        
                    tx.executeSql('UPDATE Item SET situacao = ? WHERE id = ?', [string, id]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
